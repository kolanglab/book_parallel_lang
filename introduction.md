...
もうひとつは **I/O の重ね合わせ** です。ネットワークやディスクの応答待ちは、CPU から見れば気が遠くなるほど長い「何もしない時間」です。Web サーバが何万もの接続を相手にするとき、そのほとんどは「返事を待っているだけ」で、CPU はほぼ遊んでいます。こうした **I/O バウンド** な仕事は、コアを増やしても速くなりません。代わりに、待っている間に別の仕事を進める——つまり **並行に構造化する** ことで、1 つの実行主体でも大量の接続を捌けます。これはまさに第1章で述べた「並行はあるが並列はない」状況で、第10章のイベントループがその代表的な実装です。
