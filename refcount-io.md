...
CPython の no-GIL 化（第17・21章）[](#cite:pep703) は、まさにこの「参照カウントを並列化しても遅くしない」ための工夫の集大成です。バイアス付き参照カウント、不変オブジェクトの恒久化（immortalization）、遅延カウントなどを組み合わせて、GIL を外しても単一スレッド性能を落とさないことを目指しています。「参照カウントの atomic 化の代償」がいかに重く、それを軽くするのにどれだけの工夫が要るかを示す、生きた実例です。
