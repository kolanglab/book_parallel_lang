...
> [!TIP]
> この Tiny VM は手元で実際に動かせます。スレッド数や反復回数を変え、ロックあり・なしで結果と実行時間がどう変わるかを観察してみてください。「壊れる」「遅くなる」を体感しておくと、以降の章の動機がはっきりします。第5章のこの題材は、第19章（データ競合検出）でもう一度、検出ツールにかける対象として再登場します。
