...
並列化の効果には、原理的な上限があります。それを定式化したのが **Amdahl の法則**[Amdahl の法則](#cite:amdahl1967) です。プログラムのうち並列化できる割合を `p`（残り `1-p` は本質的に直列）、コア数を `N` とすると、得られる速度向上（speedup）の上限は次式になります。
...
Amdahl の法則は悲観的に見えますが、前提に「問題サイズが固定」という仮定が隠れています。**Gustafson の法則**[Gustafson の法則](#cite:gustafson1988) は、この前提を問い直しました。
