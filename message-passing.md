...
**チャネル（channel）** は、第3章で触れた CSP[](#cite:hoare1978)に由来する、値を送受信するための管です。送り手は `send`、受け手は `receive` でやり取りし、互いの正体を知る必要はありません。
...
**アクター（actor）** は、第3章で見たとおり[](#cite:hewitt1973)、自分専用の状態と受信箱（mailbox）を持つ独立主体です。チャネルが「通信路」を第一級にするのに対し、アクターは「受け手」を第一級にします。各アクターは一度に 1 つのメッセージしか処理しないので、**自分の状態に関しては自動的に直列化** され、ロックが要りません。
...
メッセージの**順序**についても触れておきます。多くのアクターシステムは「同じ送信元から同じ受信先へ送ったメッセージの順序は保たれる」ことを保証しますが、**異なる送信元からのメッセージ同士の到着順は一般に保証されません**。さらに分散アクターでは、ネットワークの再送や障害により、順序に加えて重複や消失の扱いも設計事項になります。順序保証の範囲はアクターシステムごとに異なるので、仕様を確認して設計する必要があります。

アクターモデルが最も成功したのは Erlang/Elixir です。Joe Armstrong の学位論文[](#cite:armstrong2003)は、アクター（Erlang ではプロセスと呼ぶ）を「軽量・隔離・メッセージのみで通信・障害は監視して再起動」という設計でまとめ、電話交換機のような高信頼システムを実現しました。隔離されているからこそ、1 つのアクターがクラッシュしても他へ波及せず、監視者が再起動できる——「Let it crash（落ちたら落とせ、そして再起動せよ）」の哲学です。これは並列性そのものより、**隔離による堅牢性** が主眼であることに注意してください。

> [!NOTE]
> アクターと共有メモリは排他的ではありません。多くの処理系は、内部では共有メモリ＋スレッド（第6〜8章）で実装し、その上にアクターの抽象を載せます。アクターが「壊れない」のは、ランタイムがメッセージの受け渡し時に **値をコピーするか所有権を移す** ことで、送り手と受け手が同じ可変オブジェクトを同時に触らないよう保証するからです。この「コピーか所有権移動か」の設計は第18章の中心テーマです。
...
future の真価は **合成** にあります。`then` でつないで「結果が出たら次にこれをする」と書けると、コールバックの入れ子（いわゆるコールバック地獄）を平坦化できます。この「結果が出たら次へ」を言語構文に昇華したのが、次章で扱う `async`/`await` です。設計上の重要な仕様がもうひとつあります——`then` のコールバックを**どこで実行するか**です。future を完了させたスレッドでその場で走らせるのか、専用のエグゼキュータに積むのか、イベントループに載せるのか。完了側のスレッドで直接走らせる設計は手軽ですが、コールバックが重い処理やロック取得を行うと、完了させた側を巻き込んだ遅延やデッドロックの原因になります。future の API を設計するときは、結果の受け渡しと並んで、コールバックの実行コンテキストを仕様として明示する必要があります。
...
これらの手段はいずれも、大量の実行主体を効率よく走らせる土台——たくさんの「待ち」を安く捌ける仕組み——を必要とします。とりわけネットワークサーバのような I/O バウンドな並行では、「待っているだけ」の接続が何万も並びます。次章では、その待ちを 1 か所に集約する **イベントループと非同期 I/O** を扱い、続く第11章で、その上に同期的なコードのまま大量並行を載せる **軽量スレッドとスケジューラ** を組み上げます。
